...
class ConfigurableCommunicationPipelineExecutor(
    ...
    override suspend fun executeFromStep(request: CommunicationRequest, fromStep: String): PipelineResult {
        return withContext(MDCContext()) {
            ...
            MDC.put("useCase", request.useCase)

            try {
                logger.info("Starting pipeline execution for request ${request.id}, useCase: ${request.useCase}")

                // Get steps based on use case configuration
